...
function gerarSenha(){
	...
	if(!numeros && !simbolos && !letras_minusculas && !letras_maiusculas){
		alert('Escolha  um tipo de caracter para gerar a senha!');
		document.getElementById("senha").value = '';
		return false;
	}
	else if(document.getElementById("tamanho").value <= 0){
		alert('Escolha uma quantidade de caractéres maior que 0!');
		return false;
	}
	else if(document.getElementById("tamanho").value > 1000){
		alert('Escolha uma quantidade de caractéres menor que 1000!');
		return false;
	}
